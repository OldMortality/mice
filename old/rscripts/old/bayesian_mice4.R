##
## Binomial model, on Long version of the data, using R2jags
##
...
theData <- cbind(mouse_long,per_long,gender_long,type_1_long,
                 type_2_long,N_long,R_long,time_3_long,time_10_long,
                 time_17_long,time_24_long,time_31_long,R_long/N_long)
...
model2 <- function() 
{
  ...
  for(i in 1:N)
  {
    ...
    N_long[i] ~ dnegbin(p[mouse[i], period[i]],R_long[i])
    b[mouse[i], period[i]] ~ dnorm(0.0, tau); 
    
    log(mu[mouse[i], period[i]])<- alpha0 + alpha1 * gender[i] +
      alpha2 * type_1[i] +
      alpha3 * type_2[i] +
      alpha4 * time3[i] +
      alpha5 * time10[i] +
    ...
    p[mouse[i], period[i]] <- R_long[i]/(R_long[i] + mu[mouse[i], period[i]])
...
prop_obs <- R_long /N_long

samples <- jags(model = model2 ,
                data = list('N_long' = N_long,
                            'R_long' = R_long,
                            'period' = per_long,
                            'mouse'  = mouse_long,
                            'gender' = gender_long,
